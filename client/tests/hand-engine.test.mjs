...
test('empty space cannot click an active card',()=>{const {step}=setup();for(let n=0;n<8;n++)step([hand()],null);for(let n=0;n<25;n++)assert.equal(step([hand(.5,.5,.08)],null).clickTarget,null);});
...
test('TEST 5 pinch click fires exactly once',()=>{
 const {step}=setup();for(let n=0;n<8;n++)step([hand()]);
 let clicks=0,sawCandidate=false;
 for(let n=0;n<20;n++){const r=step([hand(.5,.5,.08)]);if(['PINCH_START','PINCH','PINCH_CANDIDATE','PINCHED'].includes(r.state)||r.pinchState==='PINCH_CANDIDATE')sawCandidate=true;clicks+=Number(!!r.clickTarget);}
 assert.ok(sawCandidate);assert.equal(clicks,1);
 for(let n=0;n<12;n++)step([hand()]);
});
test('TEST 6 held pinch still fires only one click',()=>{
 const {step}=setup();for(let n=0;n<8;n++)step([hand()]);
...
test('TEST 10 two-hand zoom out decreases scale',()=>{
 ...
 for(const d of trajectories.zoomOutSpec){const r=step(twoHands(d));if(r.zoom==null)continue;if(previous!=null)assert.ok(r.zoom<=previous+1e-6);previous=r.zoom;seen++;}
